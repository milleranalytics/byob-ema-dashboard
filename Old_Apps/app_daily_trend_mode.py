...
with col1:
    ...
    selection_method = st.selectbox(
        ...
        options=["Time Trends (Weekly)", "Time Trends (Daily)", "Average PCR"], 
        index=0,
    ...
    if "Time Trends" in selection_method:
        trend_mode = "Daily" if "Daily" in selection_method else "Weekly"
    else:
        trend_mode = None
...
def calculate_equity_curve_with_dynamic_method(
    ...
    top_times_sorted = None  # only cached for weekly mode

    for current_date in all_dates:
        if selection_method == "Average PCR":
            near_start = current_date - pd.DateOffset(months=man_near)
            mid_start = current_date - pd.DateOffset(months=man_mid)
            long_start = current_date - pd.DateOffset(months=man_long)
        ...
        elif selection_method.startswith("Time Trends"):
            if trend_mode == "Daily":
                ...
                if current_date.weekday() == 0 or top_times_sorted is None:
                    top_times_sorted = select_times_via_time_trends(
                        ema_df=ema_df,
                        end_date=current_date - pd.Timedelta(days=1),
                        num_times=num_times,
        ...
        # --- Apply time filter to today's trades
        current_day_data = ema_df[
            (ema_df['OpenDate'] == current_date) &
            (ema_df['OpenTimeFormatted'].isin(top_times_sorted or []))  # 👈 ensure valid list
        ]

        if current_day_data.empty:
            continue

        equity_at_day_start = equityStart if current_date == pd.to_datetime(start_date) else current_equity
        contracts = floor(equity_at_day_start * (risk / 100) / num_times / (average_credit * 100)) if average_credit else 0
        current_day_data = current_day_data.sort_values('OpenTimeFormatted')

        for _, trade in current_day_data.iterrows():
    ...
    if not results_df.empty and 'Date' in results_df.columns and 'Equity' in results_df.columns:
    ...
    with equity_container.container():
        with st.spinner("🔄 Calculating equity curve and performance metrics..."):
            ...
            if equity_curve.empty:
                ...
                # --- 📅 Show Entry Times Used Daily (only for Time Trends Daily)
                if trend_mode == "Daily" and not full_trades.empty:
                    unique_times_by_day = (
                        full_trades.groupby('Date')['OpenTime']
                        .unique()
                        .apply(lambda x: ', '.join(sorted(x)))
                        .reset_index()
                        .rename(columns={'OpenTime': 'SelectedTimes'})
                    )

                    with st.expander("📅 Daily Entry Times Used", expanded=False):
                        st.dataframe(unique_times_by_day, use_container_width=True)
                ...
                with expander_col:
                    with st.expander("📅 Trading Summary"):
                        if selection_method == "Average PCR":
                            ...
                            label_format = "%Y-%m"
                        elif trend_mode == "Daily":
                            period_group = full_trades['Date']
                            label_format = "%Y-%m-%d"
                        else:
                            # Weekly mode
                        ...
                        for period, trades in full_trades.groupby(period_group):
                            period_start_date = (
                                period if isinstance(period, pd.Timestamp) else
                                period.start_time if hasattr(period, 'start_time') else period
                            )

                            st.markdown(f"##### {period_start_date.strftime(label_format)}")
                            ...
                            if selection_method == "Average PCR":
                                ...
                                # Use weekly range even for daily trend mode — but only show it once per trade block
                                ...
                                if recent_trading_days:
                                    ...
                                    st.markdown(
                                        f"🔁 **Trend Ranking: {trend_mode}** Last {trend_ranking_days} days "
    ...
    for idx, slot in enumerate(pivot.columns):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                line=dict(width=1.0, color='gray'),  # <<<<< Rolling line gray
